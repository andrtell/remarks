...
We can fix this error by using the `-L` to specify the location of the shared object file `libeasymath.so` and the `-l` flag to tell the compiler to we wish to link our executable `program` with the shared object file `libeasymath.so`.
...
_Ok, lets try and run our executable `program`._
...
_A third error. It turns out that our executable `program` also need to provide the presumed location of the shared object `libeasymath.so` at runtime. This information is used by the runtime linker to load our shared object `libeasymath.so` when our executable `program` starts._
